    ...
    >
      <div className="overflow-hidden rounded-xl border border-border/60 bg-card transition-all duration-500 hover:-translate-y-1 hover:border-accent-blue/20 hover:shadow-[0_0_30px_-8px_rgba(56,189,248,0.06)]">
        ...
        <div className="p-5">
          ...
          {tags && tags.length > 0 && (
            <div className="mb-3 flex flex-wrap gap-1.5">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-md border border-border/50 px-2 py-0.5 font-mono text-[10px] tracking-wide text-muted-foreground/50"
                >
                  {tag}
                </span>
              ))}
          ...
          {/* Title */}
          <h3 className="text-base font-semibold tracking-tight text-foreground transition-colors duration-200 group-hover:text-accent-blue/90">
            {title}
          </h3>
          ...
          <div className="mt-4 flex items-center gap-2">
            <time className="font-mono text-[11px] text-muted-foreground/40">
              {formatDate(date)}
            </time>
            <ArrowUpRight className="size-3 text-muted-foreground/30 transition-all duration-300 group-hover:text-accent-blue/50 group-hover:-translate-y-px group-hover:translate-x-px" />
          </div>
        </div>
      </div>
    </Link>
