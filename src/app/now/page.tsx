...
export default function NowPage() {
  return (
    <div className="mx-auto max-w-5xl px-6 py-28 sm:py-36">
      <div className="mb-16">
        <p className="font-mono text-[10px] tracking-[0.2em] text-muted-foreground/50 uppercase">Now</p>
        <h1 className="mt-3 text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">现在</h1>
      ...
      <div className="max-w-2xl space-y-16">
          ...
          <div className="space-y-4">
            {building.map((item) => (
              <div key={item.label} className="flex items-start gap-3 rounded-xl border border-border bg-card/50 p-4">
                <Circle className={`mt-0.5 size-2 shrink-0 ${item.status === "active" ? "fill-accent-blue text-accent-blue" : "fill-muted-foreground/30 text-muted-foreground/30"}`} />
                ...
                {item.status === "paused" && (
                  <span className="ml-auto shrink-0 rounded-md border border-border px-2 py-0.5 font-mono text-[9px] text-muted-foreground/40">paused</span>
                )}
              </div>
            ))}
          </div>
        ...
        <section>
          <h2 className="mb-6 font-mono text-[10px] tracking-[0.15em] text-muted-foreground/40 uppercase">正在学习</h2>
