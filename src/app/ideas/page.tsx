    ...
    <div className="mx-auto max-w-5xl px-6 py-28 sm:py-36">
      ...
      <section className="mb-20">
        ...
        <p className="mt-4 max-w-xl text-sm leading-relaxed text-muted-foreground/70">
          关于 AI、系统、自动化与持续构建的一些记录。碎片思考、灵感与工作流片段。
        </p>
      </section>
      ...
      {hasIdeas ? (
        <div className="relative">
          {/* Timeline line */}
          <div
            className="absolute left-[19px] top-3 bottom-3 w-px lg:left-1/2 lg:-translate-x-px"
            style={{
              background:
                "linear-gradient(to bottom, rgba(56,189,248,0.15), rgba(56,189,248,0.04), rgba(56,189,248,0.15))",
            }}
          />
          ...
          <div className="space-y-16">
            {ideas.map((idea, i) => (
              <div key={idea.slug} className="relative flex items-start gap-8 lg:gap-0">
                {/* Timeline dot + date — left side on desktop */}
                <div className="relative z-10 flex shrink-0 items-center gap-4 lg:absolute lg:left-1/2 lg:-translate-x-[calc(100%+32px)] lg:flex-row-reverse lg:text-right">
                  ...
                  {/* Date */}
                  <time className="font-mono text-[11px] text-muted-foreground/40 lg:w-28 shrink-0">
                    {idea.frontmatter.date}
                  </time>
                </div>
        ...
        <div className="rounded-xl border border-border/40 bg-card/50 px-6 py-16 text-center">
          ...
          <p className="mt-2 text-xs text-muted-foreground/35">
            在 Sanity Studio 中创建第一条想法吧。
          </p>
        </div>
      )}
    </div>
